...
namespace Vistainn
{
    public partial class MReportsForm : Form
    {
        // database instance
        ...
        {
            string query = "SELECT RoomNo, SUM(Amount) AS TotalRevenue FROM payment p JOIN booking b ON p.BookingId = b.BookingId WHERE b.CheckIn BETWEEN @StartDate AND @EndDate GROUP BY RoomNo";
            var dt = GetData(query, startDate, endDate);
            PopulateChart("TotalRevenue", dt);
